...
| **web** | Next.js App Router service | `apps/web` | db, domain; browsers; provider oEmbed endpoints out; Stripe webhooks in |
...
| Flickr, Imgur, SoundCloud, Bandcamp, YouTube, Vimeo | external | — | **they** host every photo, track and video; we store a link and fetch oEmbed metadata. No object store of our own — see engineering-spec K8 (DMCA) |
...
Full route table in the M0 spec plus, as built since: `applications/[id]/status` (decline/withdraw), `bookings/[id]/{mark-played, dispute, review, agreement}`, `admin/disputes/[id]/resolve`, `media/embed` (the only media route — presign/upload/complete/file were deleted with the upload path), `ai/{profile-ingest, slot-parse, gear-extract}`, `payments/connect`, `calendar` (signed iCal), `webhooks/stripe`, `techs/list`. Conventions: zod `parseBody`; `{error:{code,message}}` failures; session = `gigit_session` httpOnly JWT (HS256, 30d, `{sub: userId}`), roles re-read from DB per request; ownership helpers (`performerOwnedBy` etc.); `isAdmin` via `actor_roles`. Rate limits: OTP 5/destination/hour; inquiries 10/user/day.
...
### 3.6 Media path
Link-only, every kind — EightGig stores no user media (engineering-spec K8/§8; the reason is DMCA §512(c) exposure). `POST media/embed`: zod + host allow-list (`EMBED_PROVIDER_HOSTS`, https-only) resolves the URL to a provider and *derives* the kind from it — photo (Flickr/Imgur), audio (SoundCloud/Bandcamp), video (YouTube/Vimeo). A URL no provider claims is rejected; the client never names the kind. Then normalize (lowercase host, no fragment) → quota check (20 photos / 10 audio / 5 video per profile) → insert `held` → enqueue `media.screen_requested`. oEmbed metadata fetch is 4s and non-fatal (Bandcamp publishes no endpoint at all). Delivery: the browser loads media straight from the provider; there is no `publicMediaUrl()`, no bucket and no media CDN. Two further allow-list checks stand behind the first — provider-response URLs are matched against that provider's asset CDNs before reaching an `<img src>` (`lib/oembed.ts`), and the CSP in `apps/web/next.config.ts` pins `img-src`/`frame-src` to the same hosts in the browser.
...
### 4.5 Media (web + worker; no storage of our own)
§3.6. **Requirements:** an arbitrary URL is never accepted or rendered — the host allow-list is enforced at the schema, again on provider-supplied asset URLs, and again by the CSP in the browser; nothing public before `ready`; per-profile link quotas at add time. **Note the deleted requirements:** EXIF strip, responsive renditions, content-type sniffing, virus scanning and S3 lifecycle rules were requirements *of holding a file*, and there is no file. **Legal review outstanding:** the hosting posture changed, so `/dmca` and `/terms` — written for a product that stored user uploads — need a pass by counsel. This design deliberately makes no claim about which obligations now apply.
...
The CDK service stack is parameterized per stage; staging is the automated target and production remains an optional, manually gated deployment. CloudFront supplies the public HTTPS origin and fronts a public ALB. The ALB accepts only the AWS-managed CloudFront origin prefix and a generated verification header, then is the only inbound source allowed to the web port on one x86 `t3.small` EC2 host in a public subnet; that host runs both linux/amd64 web and worker containers and has no SSH ingress. Direct internet-gateway egress lets the containers reach provider APIs, so there is no NAT gateway. RDS Postgres 16 is encrypted, single-AZ, in private-isolated subnets, and accepts connections only from the app host. There is no media bucket and no media CloudFront distribution — both were deleted with the link-only change, and the host role's S3 grant went with them; the host role has SES send rights and nothing else beyond ECR pull, Secrets Manager read and CloudWatch metrics.

CloudFormation generates the database credentials/`DATABASE_URL` and `SESSION_SECRET` in `AppSecrets`; provider values (`EMAIL_FROM`, `TWILIO_*`, `GEMINI_API_KEY`, `SENTRY_DSN`) remain operator-managed. The discovery-first configuration fixes `PAYMENTS_ENABLED=false` and leaves Stripe settings empty. Deployments build both images for linux/amd64 under an immutable `imageTag` and pass a unique `deploymentNonce`. A CloudFormation-managed SSM association pulls that exact tag, materializes the shared secrets, appends the actual CloudFront `APP_URL`, applies Drizzle migrations from the host, and restarts both containers. CI deploys and health-gates staging; an optional production job, if enabled, is manually gated and builds/pushes its own images before deployment. No cross-account image retag or pull is assumed. **Remaining:** complete the first staging deployment/verification, configure SES/Sentry, subscribe alarm recipients, and perform the first backup/load drills. Single-AZ is accepted per K11.

### 4.15 Configuration contract
From `env()` (zod, fail-fast): required `DATABASE_URL`, `SESSION_SECRET`; defaulted `APP_URL`, `AWS_REGION`, `GEMINI_MODEL`; optional-with-fallback `STRIPE_SECRET_KEY(+_WEBHOOK_SECRET)` → Null gateway, `GEMINI_API_KEY` → heuristic/503, `TWILIO_*`/`EMAIL_FROM` → log sink. The CDK no longer injects `STORAGE_DRIVER`/`S3_BUCKET`/`MEDIA_CDN_URL` — there is nothing to store or serve (§3.6). (`STORAGE_DRIVER`/`S3_BUCKET` remain declared in `packages/db/src/env.ts` and are now dead; removing them is a loose end outside this change.) **Design property:** the system boots and demos with *zero* external credentials; every external dependency has a local stand-in. **Known footgun:** nothing loads `.env` for `db:seed`/CLI scripts (Next loads it for web only) — export vars or add dotenv to the db package scripts.
...
| A10 | Nothing publishes media before screening | requirement | ✅ `media.screen_requested` → `media_fraud_screen` is the only path out of `held`; high risk stays held for ops |
...
4. **Media moderation surface** (F7.5, A10): the screen itself is built — `media_fraud_screen` over the link's title/provider → `fraud_flags` → `ready` or left `held`, and the weekly embed-rot recheck flags dead links to their owner. The byte-level steps this item used to list (sniff, virus scan, EXIF strip, renditions) are gone with the upload path, not deferred. **Still to build:** the ops queue UI that reads `fraud_flags` and lets a human release or block a held asset (item 5).
...
9. **Delivery operations**: first staging deploy/verification, CI OIDC variables/secrets, SES sender, alarm subscription, backup-restore drill, and load test at 100×. Immutable linux/amd64 builds, host-local migrations, two-container restart, and the staging health gate are implemented; the production build/deploy path is optional and manually gated, not evidence of a production deployment.
