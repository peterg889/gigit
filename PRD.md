...
**Gigit is a three-sided marketplace where venues post entertainment slots, performers (bands and comedians) book them, and sound techs (with or without PA rigs) attach to those bookings.** It starts as discovery and coordination — the gig feed, the booking handshake, the sound plan, reviews and reliability — and the venue and act settle up directly, the way they already do. Payments, contracts, and cancellation protection are designed in but deferred, switched on only when the scene needs them (see §4).
